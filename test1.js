...
obj.from.path("sch.csv").to.array(function (data) {
  ...
  for (var j = 26; j < 27; j++) {
    con.connect(function (err) {
      ...
      var sql =
        "INSERT INTO schol  VALUES(" +
        parseInt(MyData[j].FieldOne, 10) +
        ",'" +
        MyData[j].FieldTwo +
        "','" +
        MyData[j].FieldThree +
        ...
        MyData[j].FieldTen +
        "'," +
        parseInt(MyData[j].FieldEleven) +
        ",'" +
        MyData[j].FieldTwelve +
        "','" +
        MyData[j].FieldThirteen +
        "','" +
        MyData[j].FieldFourteen +
        "','" +
        MyData[j].FieldFifteen +
        "')";
      console.log(sql);
